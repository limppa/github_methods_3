...
```{python}
a = 2
a# a <- 2 # results in a syntax error
...
y = y.astype(float)
n_samples = len(y)
y += np.random.normal(loc=0, scale=1, size=n_samples) #add error

X = np.zeros(shape=(n_samples, 2))
X[:, 0] = x ** 0
X[:, 1] = x ** 1

b_hat = ((np.transpose(X) @ X) ** -1) @ np.transpose(X) @ y #the classic equation with transposed X matrix and ^(-1)
print(b_hat)

y_hat = b_hat[1] * x # remember that 1 index means the second value
print(y_hat)
```
...
b_hat = np.linalg.inv(np.transpose(X)@X) @ np.transpose(X) @ y # same as before, but for some reason **-1 doesn't work
```
